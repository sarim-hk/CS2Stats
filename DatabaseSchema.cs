...
namespace CS2Stats {
    ...
    public partial class Database {
        ...
        public async Task CreateTables(ILogger Logger) {
            try {
                using MySqlConnection tempConn = new(this.connString);
                await tempConn.OpenAsync();

                foreach (string statement in CreateTablesSql.Split(';', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries)) {
                    using MySqlCommand cmd = new(statement, tempConn);
                    await cmd.ExecuteNonQueryAsync();
                }

                Logger.LogInformation("[CreateTables] Database tables created or already exist.");
            }
            catch (Exception ex) {
                Logger.LogError(ex, "[CreateTables] Error occurred while creating database tables.");
            }
        }
